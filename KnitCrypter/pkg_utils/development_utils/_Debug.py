...
class _Debug(_Utility):
    ...
    def _check_flags(*targets:str) -> bool:
        ...
        _all_or_none = _Debug()._check_for_all_or_none(_active_flags)

        if _all_or_none == None:
            _Debug()._verify_flags(_active_flags)
            _Debug()._verify_flags(targets)

            return _Debug()._compare_flags(_active_flags,targets)
    ...
    def _verify_flags(flag_array:list):
        ...
        for i in flag_array:
            _Test_Cases._verify_flag(_available_flags,i)
