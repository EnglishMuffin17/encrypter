...
ENCRYPTED_STAMP = '###_________#FILE_ENCRYPTED#_________###'
...
def _untie_string(o:object,string:str):
    return "".join(_untie_chars(o._pattern,_split_tied_string(o._pattern,string)))
...
class _Needle_Struct(metaclass=ABCMeta):
    ...
    def __init__(self,super_obj,pattern,contents):
        ...
        self._is_encrypted = _verify_file_encrypted(self._contents,ENCRYPTED_STAMP)
    ...
    def erase_stamp(self):
        if self._contents[-1] == ENCRYPTED_STAMP:
...
class _Stitch_Struct(_Needle_Struct):
    ...
    def from_lines(self,*args):
        if len(args) > 2:
            raise AttributeError(f"'from_lines' accepts only 2 arguments at most")
